...
import { sleep } from './../common/helpers/test.helper';
import { randomIntFromInterval } from '../common/helpers/general.helper';
...
import { VideoUploadType, VideoStoragePrefix } from '../common/dto';
import { ThumbnailHelper, VideoHelper, VideoPath } from './videos.helper';
...
export class VideosService {
  ...
  async getByIds(ids: string[], userId?: string) {
    const videos = await this.videosRepository.getByIds({ userId, ids });
  ...
  async create(userId: string) {
    const video = await this.videosRepository.create({ userId });
    return {
      id: video.id,
    };
  }
  ...
  async filter(filters: { status: UPLOAD_VIDEO_STATUS }) {
    return this.videosRepository.filter({ status: filters.status });
  }

  async gets() {
    return this.videosRepository.filter();
  }

  async update(id: string, updateUploadDto: UpdateVideoDto): Promise<boolean> {
    await this.videosRepository.updateById(id, updateUploadDto);
    return true;
  }
  ...
  async deleteByIds(ids: string[], userId?: string): Promise<void> {
    ...
    await this.videosRepository.deleteByIds(ids, userId);
  ...
  async markVideosHasBeenUsed(ids: string[], userId?: string): Promise<void> {
    ...
    await this.videosRepository.markVideosHasBeenUsed(ids);
  ...
  async deleteVideosInObjectStorage(videoIds: string[]) {
    const videoUrls = [];
    const thumbnailUrls = [];
    const videos = await this.videosRepository.getByIds({
      ids: videoIds,
    ...
    if (videoUrls.length > 0) {
      await this.objectStorageService.deleteVideos(videoUrls);
    }

    if (thumbnailUrls.length > 0) {
      await this.objectStorageService.deleteThumbnails(thumbnailUrls);
    }
  }
  ...
  async handleTranscodeDone(videoId: string, vimeoId: string) {
    try {
      ...
      for (const mp4Info of mp4Infos) {
        ...
        const { src } = await this.objectStorageService.uploadVariantVideo(
          mp4Info.link,
          videoId,
          mp4Info.rendition,
        );
      ...
      const { props, properties, thumbnails, userId, status } =
        await this.videosRepository.getById(videoId);
      // update db
      await this.videosRepository.updateById(videoId, {
        files,
      ...
      if (status != UPLOAD_VIDEO_STATUS.DONE) {
        this.kafkaService.public<VideoProcessingEndDto>(
  ...
  async processing(videoId: string) {
    try {
      ...
      await this.objectStorageService.downloadVideoFromS3(key, localFilePath);
      ...
      this.videosRepository.updateById(videoId, {
  ...
  async upload(
    ...
    try {
      const video = await this.videosRepository.getByIdAsUser(videoId, userId);
      ...
      const s3upload = await this.objectStorageService.uploadOriginVideo(
        videoFullPath,
        videoId,
        file.originalname,
        videoUploadType,
      ...
      await this.videosRepository.updateById(videoId, record);
  ...
  async initIntervalCheckProcessing() {
    try {
      ...
      const videos = await this.videosRepository.filter({
        status: UPLOAD_VIDEO_STATUS.PROCESSING,
      });
  ...
  async generateThumbnailsAndUploadToS3(
    ...
    for (const thumbnail of thumbnails) {
      ...
      const s3upload = this.objectStorageService.uploadThumbnailToS3(
        thumbnail.path,
        key,
      );
  ...
  async deleteVideosNotUse(maxAgeInHour?: number) {
    try {
      let offset = 0;
      while (true) {
        const videos = await this.videosRepository.getVideosNotUse({
          offset,
  ...
  async processVideo(bucket: string, key: string) {
    ...
    try {
      ...
      await this.objectStorageService.downloadVideoFromS3(
        key,
      ...
      const presignedUrlVideo = await this.objectStorageService.getPresignedUrl(
        videoPath.getOriginPath(),
        this.s3Config.userUploadVideosBucket,
        {
          ExpiresInSecond: VIDEO_PRESIGNED_POST_EXPIRED,
